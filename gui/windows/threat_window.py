...
from gui.windows.architecture import format_diagram_name
from analysis.safety_management import SAFETY_ANALYSIS_WORK_PRODUCTS
from gui.dialogs.threat_dialog import ThreatDialog


def _join_damage_scenarios(entry: ThreatEntry) -> str:
    """Return concatenated damage scenarios for ``entry``."""
    return "; ".join(
        ds.scenario for f in entry.functions for ds in f.damage_scenarios
    )


def _join_damage_types(entry: ThreatEntry) -> str:
    """Return concatenated damage types for ``entry``."""
    return "; ".join(
        ds.dtype for f in entry.functions for ds in f.damage_scenarios
    )


def _join_threats(entry: ThreatEntry) -> str:
    """Return concatenated threat scenarios for ``entry``."""
    return "; ".join(
...
def _join_attack_paths(entry: ThreatEntry) -> str:
    """Return concatenated attack paths for ``entry``."""
    return "; ".join(
        ap.description
        for f in entry.functions
        for ds in f.damage_scenarios
...
class ThreatWindow(tk.Frame):
    ...
    def refresh(self):
        ...
        for idx, entry in enumerate(self.app.threat_entries):
            ...
            damages = _join_damage_scenarios(entry)
            types = _join_damage_types(entry)
            threats = _join_threats(entry)
            paths = _join_attack_paths(entry)
            self.tree.insert(
                "",
                "end",
                iid=str(idx),
