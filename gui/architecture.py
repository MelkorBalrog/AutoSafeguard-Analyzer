...
class SysMLObjectDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        ...
        if self.obj.obj_type == "Block":
            ...
            link_row += 1
            # Trace options inserted after type-specific links
            ...
            link_row += 1
        # Trace targets based on governance-defined relationships
        trace_opts: list[str] = []
        if toolbox:
            diag_name = ""
            if self.obj.obj_type == "Work Product":
                diag_name = self.obj.properties.get("name", "")
            elif current_diagram:
                diag_name = current_diagram.name or ""
            wp = wp_map.get(diag_name)
            if wp:
                trace_opts = sorted(getattr(wp, "traceable", []))
        if trace_opts:
