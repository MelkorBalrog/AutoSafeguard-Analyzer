...
Each underlying `Fault` is linked to its resulting requirements, and the FTA root node references the associated `SafetyGoal` so mitigations trace back to the HARA results.
...
classDiagram
    ...
    SafetyGoal --> FaultTreeNode : topEvent
    FmeaEntry --> FaultTreeNode : baseEvent
    FaultTreeDiagram --> "*" FaultTreeNode : nodes
    FaultTreeNode --> Requirement : requirement
    Fault --> Requirement : mitigatedBy
