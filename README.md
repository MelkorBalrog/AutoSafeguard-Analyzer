...
missing before handing off execution to `AutoML/auto_ml_app.py`. The required packages
are:
...
If double‑clicking `AutoML/auto_ml_app.py` closes immediately, launch it from a command
...
1. Install the required packages:
   ...
   pip install pillow openpyxl networkx matplotlib reportlab adjustText
   ```
2. Launch AutoML with `python AutoML/auto_ml_app.py` and choose **File → New Project**.
