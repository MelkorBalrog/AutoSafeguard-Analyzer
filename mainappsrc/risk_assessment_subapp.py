...
class RiskAssessmentSubApp:
    ...
    # ------------------------------------------------------------------
    # Helper lookups and status propagation
    # ------------------------------------------------------------------
    def get_hazop_by_name(self, app, name):
        for d in app.hazop_docs:
            if d.name == name:
                return d
        return None

    def get_hara_by_name(self, app, name):
        for d in app.hara_docs:
            if d.name == name:
                return d
        return None
    ...
    def get_safety_goal_asil(self, app, sg_name):
        ...
        for te in app.top_events:
            if sg_name and (
                sg_name == te.user_name or sg_name == te.safety_goal_description
            ):
    ...
    def get_safety_goals_for_malfunctions(self, app, malfunctions):
        goals = []
        for te in app.top_events:
            mal = getattr(te, "malfunction", "")
    ...
    # ------------------------------------------------------------------
    # Repository management for malfunctions, hazards and lists
    # ------------------------------------------------------------------
    def is_malfunction_used(self, app, name):
        if not name:
            return False
        for te in app.top_events:
    ...
    def add_malfunction(self, app, name):
        app.push_undo_state()
        if not name:
            return
        name = name.strip()
        ...
        if not exists and not any(
            getattr(te, "malfunction", "") == name for te in app.top_events
        ):
    ...
    def add_fault(self, app, name):
        app.push_undo_state()
        append_unique_insensitive(app.faults, name)

    def add_failure(self, app, name):
        app.push_undo_state()
        append_unique_insensitive(app.failures, name)

    def add_hazard(self, app, name, severity=1):
        app.push_undo_state()
    ...
    def add_triggering_condition(self, app, name):
        app.push_undo_state()
        name = (name or "").strip()
    ...
    def delete_triggering_condition(self, app, name):
        app.push_undo_state()
    ...
    def rename_triggering_condition(self, app, old, new):
        app.push_undo_state()
    ...
    def add_functional_insufficiency(self, app, name):
        app.push_undo_state()
        name = (name or "").strip()
    ...
    def delete_functional_insufficiency(self, app, name):
        app.push_undo_state()
    ...
    def rename_functional_insufficiency(self, app, old, new):
        app.push_undo_state()
        if not old or old == new:
            return
    ...
    def rename_malfunction(self, app, old, new):
        app.push_undo_state()
        if not old or old == new:
            return
        ...
        for d in app.fmedas:
            for e in d.get("entries", []):
                self._replace_entry_mal(e, old, new)
        app.update_views()
        self._update_shared_product_goals(app)
    ...
    def _update_shared_product_goals(self, app):
        groups = {}
        for te in app.top_events + getattr(app, "cta_events", []) + getattr(app, "paa_events", []):
            mal = getattr(te, "malfunction", "")
            if mal:
                groups.setdefault(mal, []).append(te)
        app.shared_product_goals = getattr(app, "shared_product_goals", {})
        for mal, events in groups.items():
            if len(events) > 1:
                pg = app.shared_product_goals.get(mal)
                if not pg:
                    pg = {"name": events[0].user_name}
                    app.shared_product_goals[mal] = pg
                for e in events:
                    e.user_name = pg["name"]
                    e.name_readonly = True
                    e.product_goal = pg
            else:
                app.shared_product_goals.pop(mal, None)
                ev = events[0]
                ev.name_readonly = False
                ev.product_goal = None

    def rename_hazard(self, app, old, new):
        app.push_undo_state()
        if not old or old == new:
            return
    ...
    def update_hazard_severity(self, app, hazard, severity):
        app.push_undo_state()
        try:
            severity = int(severity)
        except Exception:
    ...
    def rename_fault(self, app, old, new):
        app.push_undo_state()
        if not old or old == new:
            return
    ...
    def rename_failure(self, app, old, new):
        app.push_undo_state()
        if not old or old == new:
            return
    ...
    def _replace_name_in_list(self, value, old, new):
        parts = []
        changed = False
    ...
    def _remove_name_from_list(self, value, name):
        parts = []
        for p in value.split(";"):
            p = p.strip()
            if p and p != name:
