...
def test_pmfh_autopopulates_validation_target_and_probability(monkeypatch):
    ...
    spi_tree = app._spi_tree
    spi_iid = next(iter(spi_tree.data))
    assert spi_tree.data[spi_iid]["values"][1] == f"{PMHF_TARGETS['D']:.2e}"
    assert spi_tree.data[spi_iid]["values"][2] == f"{2e-6:.2e}"
    assert spi_tree.data[spi_iid]["values"][3] == f"{expected_spi:.2f}"
