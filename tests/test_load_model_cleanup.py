...
def test_load_model_invokes_reset(tmp_path, monkeypatch):
    ...
    app._prompt_save_changes_v4 = MagicMock(return_value=True)
    ...
    app.load_model()
    app._prompt_save_changes_v4.assert_called_once()
...
def test_load_model_cancel_aborts(tmp_path, monkeypatch):
    model = tmp_path / "model.json"
    model.write_text("{}")
    ...
    app.set_last_saved_state = MagicMock()
    app._reset_on_load = MagicMock()
    app._prompt_save_changes_v4 = MagicMock(return_value=False)
...
@pytest.mark.parametrize(
    "variant,attr",
    [
        ("_prompt_save_changes_v1", "askyesno"),
        ("_prompt_save_changes_v2", "askyesnocancel"),
        ("_prompt_save_changes_v3", "askokcancel"),
        ("_prompt_save_changes_v4", "askyesnocancel"),
    ],
)
def test_prompt_variants_call_save(monkeypatch, variant, attr):
    app = AutoMLApp.__new__(AutoMLApp)
    monkeypatch.setattr(app, "has_unsaved_changes", lambda: True)
    called = {}
    def fake_save():
        called["saved"] = True
    app.save_model = fake_save
    monkeypatch.setattr(AutoML.messagebox, attr, lambda *a, **k: True)
    assert getattr(app, variant)() is True
    assert called
